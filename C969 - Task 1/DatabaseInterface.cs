...
namespace C969___Task_1
{
    static class DatabaseInterface
    {
        ...
        static public int CustomerRecordAdd(string name, int addressId)
        {
            if (addressId == 0)
            {
                throw new Exception(Language.LanguageFill("#internalerror address ID is zero."));
            }
            int customerId = 0;
            MySqlConnection connection = Session.GetDBConnection();
            MySqlCommand command = connection.CreateCommand();
            ...
            if (command.ExecuteNonQuery() == 1)
            {
                ...
                using (MySqlDataReader reader = command.ExecuteReader())
                {
                    ...
                    {
                        throw new Exception(Language.LanguageFill("#internalerror retrieving new customer ID from database"));
                    }
                }
            }
            else
            {
                throw new Exception(Language.LanguageFill("#internalerror inserting new customer record."));
            }
            command.Dispose();
            connection.Dispose();
            return customerId;
                ...
                else
                {
                    ...
                    if (command.ExecuteNonQuery() == 1)
                    {
                        ...
                        using (MySqlDataReader insertIdReader = command.ExecuteReader())
                        {
                            ...
                            {
                                throw new Exception(Language.LanguageFill("#internalerror occurred while retrieving new Address ID from database."));
                            }
                        }
                    }
                    else
                    {
                        throw new Exception(Language.LanguageFill("#internalerror occurred while creating address record."));
                    }
                }
            }
            command.Dispose();
        ...
        static public List<string> CityList(string country)
        {
            ...
            {
                throw new Exception(Language.LanguageFill("#internalerror occurred retrieving city list: " + e.Message));
            }
            return cityList;
        }
        static public List<string> CountryList()
        {
            ...
            {
                throw new Exception(Language.LanguageFill("#internalerror occurred retrieving country list: " + e.Message));
            }
            return countryList;
        }
                ...
                else
                {
                    ...
                    if (command.ExecuteNonQuery() == 1)
                    {
                        ...
                        using (MySqlDataReader insertIdReader = command.ExecuteReader())
                        {
                            ...
                            {
                                throw new Exception(Language.LanguageFill("#internalerror occurred while retrieving new City ID from database."));
                            }
                        }
                    }
                    else
                    {
                        throw new Exception(Language.LanguageFill("#internalerror occurred while creating city record."));
                    }
                }
            }
            command.Dispose();
        ...
        static public int CityAdd(string cityCountryString)
        {
            ...
            {
                throw new Exception(Language.LanguageFill("#internalerror String should match format [City, Country]"));
            }
            return cityId;
        }
                ...
                else
                {
                    ...
                    if (command.ExecuteNonQuery() == 1)
                    {
                        ...
                        using (MySqlDataReader insertIdReader = command.ExecuteReader())
                        {
                            ...
                            {
                                throw new Exception(Language.LanguageFill("#internalerror occurred while retrieving new Country ID from database."));
                            }
                        }
                    }
                    else
                    {
                        throw new Exception(Language.LanguageFill("#internalerror occurred creating country record."));
                    }
                }
            }
            command.Dispose();
        ...
        public static List<Customer> CustomerList()
        {
            ...
            using (MySqlDataReader reader = command.ExecuteReader())
            {
                while (reader.Read())
                {
                    customers.Add(new Customer(reader.GetInt32("customerId")
            ...
            using (MySqlConnection connection = Session.GetDBConnection())
            {
                ...
                using (MySqlDataReader reader = command.ExecuteReader())
                {
                    ...
                    else
                    {
                        ...
                        {
                            throw new Exception(Language.LanguageFill("#internalerror inserting user " + userName));
                        }
                    }
                }
            }
        ...
        public static bool UserGet(string userName, out int userId)
        {
            ...
            else
            {
                ...
                command.Dispose();
                connection.Dispose();
                throw new Exception(Language.LanguageFill("#usernotfound"));
            }
        }
        ...
        static public Dictionary<int, Customer> CustomerDictionary(bool activeOnly)
        {
                    ...
                    using (MySqlDataReader reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            customerDictionary.Add(reader.GetInt32("customerId")
                                                    ,new Customer(reader.GetInt32("customerId")
                                                        , reader.GetString("customerName")
                                                        , addressDictionary[reader.GetInt32("addressId")]
                                                        )
                                                    );
        ...
        static public List<Appointment> AppointmentList(int? userId, DateTime start, DateTime end)
        {
            ...
            Dictionary<int, Customer> customerDictionary = CustomerDictionary(false);
        ...
        static public int AppointmentRecordSet(int? appointmentId, int customerId, int userId, string title, string description, string location, string contact, string type, string url, DateTime start, DateTime end)
        {
            ...
            {
                throw new Exception(Language.LanguageFill("#appointmentconflict"));
            }
            using (MySqlConnection connection = Session.GetDBConnection())
            {
                MySqlCommand command = connection.CreateCommand();
                ...
                if (command.ExecuteNonQuery() == 1)
                {
                    if (!appointmentId.HasValue)
                    {/*
                        command.CommandText = "select last_insert_id() appointmentId";
                        using (MySqlDataReader reader = command.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                _appointmentId = reader.GetInt32("appointmentId");
                            }
                            else
                            {
                                throw new Exception("Error retrieving new appointment ID from database");
                            }
                        }*/
        ...
        static public void AppointmentRecordGet(int appointmentId, out int customerId, out int userId, out string title, out string description, out string location, out string contact, out string type, out string url, out DateTime start, out DateTime end)
        {
            using (MySqlConnection connection = Session.GetDBConnection())
            {
                ...
                using (MySqlDataReader reader = command.ExecuteReader())
                {
                    ...
                    {
                        throw new Exception(Language.LanguageFill("#internalerror #appointment #notfound"));
                    }
                }
                command.Dispose();
            }
        }
        static public string AdHocQueryToHTMLTable(string query, params object[] sqlParams)
        {
        ...
        static public List<string> AdHocQueryToStringList(string query)
        {
            ...
            using (MySqlConnection connection = Session.GetDBConnection())
            {
                ...
                using (MySqlDataReader reader = command.ExecuteReader())
                {
                    ...
                    {
                        throw new Exception(Language.LanguageFill("#toomanycolumns"));
                    }
                    //read from the reader...
                    while (reader.Read())
                    {
